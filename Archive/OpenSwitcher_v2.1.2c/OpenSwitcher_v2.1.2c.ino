...
int swap_drives(const int mode)
{
  ...
  if (mode == 1) 
  {
    detachInterrupt(switch_request);
    standby(0);
  }
  else
  return res;
...
void loop() {
  ...
  if (digitalRead(switch_request) == HIGH)
  {
    ...
    if (res == 1)
    {
      if (swap_drives(0) == 1) swapactive = true; else swapactive = false;
        //led_flash (64, 64, 64, 10, 25000, 25000);
        reset();
    }
    else if (res == 2)
    ...
    if (relaylatching)
    {
      if (!coil1_active)
      {
        ...
        coil2_active = false;
            ...
            else led_flash (64, 0, 0,1,100000,0);
    }
    else
    {
  ...
  attachInterrupt(cia_sel0, green_led, LOW);
  attachInterrupt(cia_sel1, blue_led, LOW);
  attachInterrupt(cia_sel0, black_led, HIGH);
  attachInterrupt(cia_sel1, black_led, HIGH);
#endif
  //delayMicroseconds(500000);
...
//  }
    if (swapactive)
    {
      ...
      led_flash (0, 0, 64, 10, 25000, 25000);
    }
    else
    {
      if (relaylatching)
      ...
      led_flash (0, 64, 0, 10, 25000, 25000);
    }
    attachInterrupt(switch_request, manual_swap_drive, LOW);
    standby(2);
#endif
  led_show(0, 0, 0);
}
